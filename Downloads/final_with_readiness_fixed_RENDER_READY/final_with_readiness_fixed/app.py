...
engine = db_url = app.config['DATABASE_URL']
# Ensure SQLite directory exists if using sqlite URL
db_file = None
...
engine = create_engine(db_url, connect_args={"check_same_thread": False} if db_file else {}, pool_pre_ping=True)
SessionLocal = sessionmaker(bind=engine, future=True)

# users table
# (moved to init_db to avoid connecting at import time)
...
# --- Lazy DB initialization (Render-safe) ---
_db_initialized = False
def init_db():
    global _db_initialized
    if _db_initialized:
        return
    # TODO: create tables if needed, e.g.:
    # with engine.begin() as conn:
    #     conn.execute(text("""
